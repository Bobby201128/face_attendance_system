...
def integrate_mobile_ui():
    ...
    if not os.path.exists(mobile_html_path):
        print(f"✗ 找不到文件: {mobile_html_path}")
        return False

    if not os.path.exists(new_features_path):
        print(f"✗ 找不到文件: {new_features_path}")
        return False
    ...
    if re.search(script_end_pattern, mobile_html):
        ...
        print("✓ 新JavaScript代码已添加")
    else:
        print("✗ 无法找到script结束标签")
        return False
    ...
    print(f"✓ 文件已保存: {mobile_html_path}")

    # 总结
    print_section("集成完成")
    print("✓ 所有组件已成功集成到mobile.html")
    print()
    print("新增功能:")
